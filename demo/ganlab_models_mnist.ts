...
export class GANLabModel {
  ...
  initializeModelVariables() {
    ...
    const gfc0W = tf.variable(
      tf.randomNormal(
        [this.noiseSize + this.numClasses, this.numGeneratorNeurons], 0, 1.0 / Math.sqrt(2)));
    ...
    const dfc0W = tf.variable(
      tf.randomNormal(
        [2 + this.numClasses , this.numDiscriminatorNeurons], 0, 1.0 / Math.sqrt(2)),
      true);
  ...
  generator(noiseTensor: tf.Tensor2D, labelsTensor: tf.Tensor2D): tf.Tensor2D {
    const gfc0W = this.gVariables[0] as tf.Tensor2D;
    const gfc0B = this.gVariables[1];

    let network = noiseTensor.concat(labelsTensor,1).matMul(gfc0W)
      .add(gfc0B)
      .relu();
  ...
  discriminator(inputTensor: tf.Tensor2D, labelsTensor:tf.Tensor2D): tf.Tensor1D {
    const dfc0W = /*this.*/dVariables[0] as tf.Tensor2D;
    const dfc0B = /*this.*/dVariables[1];

    let network = inputTensor.concat(labelsTensor,1).matMul(dfc0W)
      .add(dfc0B)
      .relu();
